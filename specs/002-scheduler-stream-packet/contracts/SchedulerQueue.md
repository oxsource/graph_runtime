...
- `AddNode()` pushes an `Item` onto the priority queue and, if running, calls `executor_->AddTask(this)` to submit work to the executor. When not running (paused), the item stays queued and is submitted by `SubmitWaitingTasksToExecutor()` on `Resume()`.
...
- **Event-driven drain (MediaPipe parity)**: after each `Process` invocation, `RunNode()` re-adds the node while any input packet remains buffered (`HasPendingInput(node)`). This is the `EndScheduling → SchedulingLoop` pattern: a batch (e.g. packets emitted by an encoder during Flush) drains one packet per invocation without the scheduler scanning input queues. Once all inputs are empty no re-scheduling happens, so the queue becomes idle and `HandleIdle` can terminate the graph. The same guard applies after a node returns `StatusStop`.
...
```
Node becomes ready
  → arrival callback / post-process re-scheduling → node->GetSchedulerQueue()->AddNode(node)
    → queue_.push(Item)
    → executor_->AddTask(this)
      → Schedule([this]{ RunNextTask(); })
        → executor thread: RunNextTask()
          → pop Item → RunNode()
            → pop packet(s) → node->Process() → PostProcess()
